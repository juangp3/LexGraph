...
export class PgWordDetailsRepository implements WordDetailsRepository {
  private async getWordDetailsByIds(wordIds: string[]): Promise<Map<string, WordDetails>> {
    if (!wordIds.length) {
      return new Map();
    }
    ...
    const detailsById = new Map<string, WordDetails>();
  ...
  async getWordDetails(wordId: string): Promise<WordDetails | null> {
    const details = await this.getWordDetailsBatch([wordId]);
    return details[0] ?? null;
  }

  async getWordDetailsBatch(wordIds: string[]): Promise<Array<WordDetails | null>> {
    const uniqueWordIds = [...new Set(wordIds.filter(Boolean))];
    if (uniqueWordIds.length === 0) {
      return [];
    }

    const detailsById = await this.getWordDetailsByIds(uniqueWordIds);
    return uniqueWordIds.map((wordId) => detailsById.get(wordId) ?? null);
  }
}
